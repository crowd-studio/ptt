...
class PttFormFieldTypeUpload extends PttFormFieldType
{
    public function field()
    {
        ...
        $prefix = $s3['prodUrl'] . strtolower($this->entityInfo->getEntityName() . '-' . $this->field->name) . '/';
        ...
        $html .= '<input class="hidden" type="hidden" ' . $this->attributes() .' data-produrl="'. $s3['prodUrl'] .'" data-key="'. $s3['accessKey'] .'" data-folder="'. strtolower($this->entityInfo->getEntityName() . '-'. $this->field->name) . '"  data-bucket="' . $s3['bucket'] . '" data-signer="' . $s3['signer'] . '" data-region="' . $s3['region'] . '" value="'. $this->value .'" />';
